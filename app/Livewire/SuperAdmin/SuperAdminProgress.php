...
class SuperAdminProgress extends Component
{
    ...
    public function fetchProgress()
    {
        ...
        $studentTotalTasks = Tasks::where('group_no',$this->group_no)->count();
        $studentCompletedTasks = Tasks::where('group_no',$this->group_no)
            ->whereNotNull('task_completed_date')
            ->count();
        $studentPendingTasks = Tasks::where('group_no',$this->group_no)
        ...
        $instructorTotalTasks = Tasks::where('group_no',$this->group_no)->count();
        $instructorCompletedTasks = Tasks::where('group_no',$this->group_no)
            ->where('task_completed_date', '!=',null)
            ->where('task_status', 1)
            ->count();
        $instructorPendingTasks = Tasks::where('group_no',$this->group_no)
    ...
    public function mount($group_no){
        ...
        $this->participants_count = User::where('group_no',$this->group_no)->count();
        $this->notification_count = Notifications::where('group_no',$this->group_no)->count();
        $this->repositories_count = Repositories::where('group_no',$this->group_no)->count();
        $this->tasks_count = Tasks::where('group_no',$this->group_no)->count();

        $this->accepted_proposal = Proposals::where("group_no",$group_no)->where("is_accepted",1)->first();
    ...
    {
        return view('livewire.super-admin.super-admin-progress');
    }
}
